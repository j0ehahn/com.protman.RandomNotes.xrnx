...
function Random:_neighbour(note)
  ...
  if self.preserve_notes and Random.note_sets[self.mode]:find(prefix) ~= nil then
    -- Keep note
  else
...
function Iterator:__init(constrain_to_selected, ignore_muted, callback_function)
  ...
  self.callback = callback_function or function(x)
      return x
    end
end
...
function Iterator:_do(pos, line)
  if
    (not self.ignore_muted or
      self.ignore_muted and renoise.song().tracks[pos.track].mute_state == renoise.Track.MUTE_STATE_ACTIVE) and
      not line.is_empty
   then
    for _, note_col in ipairs(line.note_columns) do
      if
        (not self.constrain_to_selected or self.constrain_to_selected and note_col.is_selected) and
          (not note_col.is_empty and note_col.note_value ~= renoise.PatternTrackLine.NOTE_OFF and
...
function invoke_random(
  ...
  if (preserve_octave == nil) then
    preserve_octave =
      (renoise.app():show_prompt("Randomizer", "Preserve the octave of each note?", {"No", "Yes"}) == "Yes")
  end

  local randomizer = Random(mode)
  if key then
    randomizer:set_key(key)
  end
  if preserve_octave then
    randomizer:set_preserve_octave(preserve_octave)
  end
  if preserve_notes then
    randomizer:set_preserve_notes(preserve_notes)
  end
  if neighbour then
    randomizer:set_neighbour(neighbour, shift)
  end
  if min and max then
    randomizer:set_range(min, max)
  end

  local iterator = Iterator(constrain, ignore_muted)
  iterator:set_callback(
    function(x)
      return randomizer:randomize(x)
    end
  )
...
function genbass1()
  --print('hello')
  --clear selection
  --generate random note for every row in selection based on note array
  --generate random vol 41-80
  --generate note-offs
  --generate rests
  --generate glides w/ random vol 1-40
  --remove inst from glides
  ...
  for pos, line in pattern_iter:note_columns_in_pattern(pattern_index, track_index) do
    if line.is_selected then --only selection
      renoise.song().selected_pattern_track:line(pos.line):clear()
      renoise.song().selected_pattern_track:line(pos.line):note_column(1).note_value = math.random(0, 120)
      renoise.song().selected_pattern_track:line(pos.line):note_column(1).volume_value = math.random(128, 128)
      renoise.song().selected_pattern_track:line(pos.line):note_column(1).instrument_value =
  ...
  for pos, line in pattern_iter:note_columns_in_pattern(pattern_index, track_index) do
    if line.is_selected then --only selection
      bopt = math.random(1, 4)
      if (bopt == 1) then
      --keep note
      end
      if (bopt == 2) then
        renoise.song().selected_pattern_track:line(pos.line):clear()
        renoise.song().selected_pattern_track:line(pos.line):note_column(1).note_value = 120
      --note off
      end
      if (bopt == 3) then
        renoise.song().selected_pattern_track:line(pos.line):clear()
      ...
      if (bopt == 4) then
        cn = renoise.song().selected_pattern_track:line(pos.line):note_column(1).note_value
        --note with glide, blank inst, vol 1-4
        renoise.song().selected_pattern_track:line(pos.line):note_column(1).instrument_string = ""
        renoise.song().selected_pattern_track:line(pos.line):note_column(1).volume_string = "70"
        renoise.song().selected_pattern_track:line(pos.line):effect_column(1).number_string = "0G"
        glide = math.abs(tn - cn) * 16
        if (glide > 255) then
          glide = 255
        end
        renoise.song().selected_pattern_track:line(pos.line):effect_column(1).amount_value = glide
      end
      tn = cn
    end
  end
...
function genbass2()
  ...
  for pos, line in pattern_iter:note_columns_in_pattern(pattern_index, track_index) do
    if line.is_selected then --only selection
      --renoise.song().selected_pattern_track:line(pos.line):effect_column(1).number_string = ""
      --renoise.song().selected_pattern_track:line(pos.line):effect_column(1).amount_value = 0
      renoise.song().selected_pattern_track:line(pos.line):note_column(1).volume_string = ""
      --renoise.song().selected_pattern_track:line(pos.line):note_column(1).instrument_value = renoise.song().selected_instrument_index - 1
      if (renoise.song().selected_pattern_track:line(pos.line):effect_column(1).amount_value > 0) then
        renoise.song().selected_pattern_track:line(pos.line):effect_column(1).amount_value = 255
      end
      if (renoise.song().selected_pattern_track:line(pos.line):note_column(1).note_value < 120) then
        renoise.song().selected_pattern_track:line(pos.line):note_column(1).instrument_value =
          renoise.song().selected_instrument_index - 1
      end
    end
